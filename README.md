...
Optional knowledge base integration: this skill works without any external personal knowledge base. If you already have your own AI fundamentals, AI product management, industry domain, customer delivery, or engineering notes, you can mention the path, title, or summary during onboarding. The skill can use that material as background context for explanations, examples, and practice tasks. Product-manager-oriented AI knowledge is useful conceptual grounding, but it should not be treated as a substitute for hands-on engineering, deployment, observability, and production troubleshooting practice.
...
可选知识库接入：这个 skill 不要求用户提前拥有任何个人知识库。如果你已经有自己的 AI 通识、AI 产品经理、行业领域、客户交付或工程笔记，可以在 Day 1 摸底时告诉助手知识库的路径、标题或摘要。后续每日学习可以把这些资料作为解释、案例和练习任务的背景。如果你的知识库偏产品经理视角，它适合作为概念理解和产品判断的基础，但不能替代工程实现、部署、可观测性和生产故障排查训练。
